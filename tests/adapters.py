...
def get_tokenizer(
    ...
    return BPETokenizer(BPETokenizerParams(vocab=vocab, merges=merges_dict), special_tokens=special_tokens)
    ...
    Returns:
        Tuple of (vocab, merges):
            ...
            merges: list[tuple[bytes, bytes]]
                ...
                Merges are ordered by order of creation.
    """
    output = train_bpe(input_path, vocab_size, special_tokens)
    vocab = output.vocab
    merges = [tuple(vocab[b] for b in pair) for pair in output.merges.keys()]
    return vocab, merges
